...
export async function fetchGlobalAllData({
  ...
  const data = fillDataDefaults(deepClone(cachedData))
...
function fillDataDefaults(db) {
  const definedData = Object.fromEntries(
    Object.entries(db || {}).filter(([, value]) => value !== undefined)
  )
  return {
    ...EmptyData(BLOG.NOTION_PAGE_ID),
    ...definedData
  }
}
...
function getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG }) {
  const defaultTitle =
    NOTION_CONFIG?.TITLE || BLOG.TITLE || `${BLOG.AUTHOR} BLOG`
  ...
  const lockPersonalIdentity = BLOG.THEME === 'loonge'
  const title = lockPersonalIdentity
    ? BLOG.TITLE
    : collection?.name?.[0][0] || defaultTitle
  const description = lockPersonalIdentity
    ? BLOG.DESCRIPTION
